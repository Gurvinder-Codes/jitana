...
static void seed_placeholder_classes(virtual_machine& vm) {
    static const char* kBootstrapTypes[] = {
        ...
        // Bulk registration: every framework superclass/interface referenced
        // by the TaintBench corpus (39 real-world malware APKs) that was NOT
        // already registered above. Each of these is the same class-loading
        // bug as SQLiteOpenHelper: dex_file::load_class silently drops a
        // WHOLE class if any of its superclasses/interfaces can't be
        // resolved, so a user class extending e.g. DeviceAdminReceiver or
        // ContentObserver (both common in this malware's spyware/locker
        // behavior) never appeared in any analysis output. Found by
        // dexdump-scanning all 39 APKs for "Superclass:" values not backed
        // by a class_def_item in the dex and not already in this list.
